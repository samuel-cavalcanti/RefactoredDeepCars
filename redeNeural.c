...
void RNA_CalcularSaida(RedeNeural *Rede) {
    ...
    for (k = 1; k < Rede->QuantidadeEscondidas; k++) {
        for (i = 0; i < Rede->CamadaEscondida[k].QuantidadeNeuronios - BIAS; i++) {
            ...
            for (j = 0; j < Rede->CamadaEscondida[k - 1].QuantidadeNeuronios; j++) {
                Somatorio = Somatorio + Rede->CamadaEscondida[k - 1].Neuronios[j].Saida *
                                        Rede->CamadaEscondida[k].Neuronios[i].Peso[j];
            }
    ...
    for (i = 0; i < Rede->CamadaSaida.QuantidadeNeuronios; i++) {
        ...
        for (j = 0; j < Rede->CamadaEscondida[k - 1].QuantidadeNeuronios; j++) {
            Somatorio = Somatorio +
                        Rede->CamadaEscondida[k - 1].Neuronios[j].Saida * Rede->CamadaSaida.Neuronios[i].Peso[j];
        }
...
RedeNeural *RNA_CriarRedeNeural(int QuantidadeEscondidas, int QtdNeuroniosEntrada, int QtdNeuroniosEscondida,
