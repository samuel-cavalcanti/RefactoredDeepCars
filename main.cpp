...
inline char *to_c_str(std::string string) {
    ...
    for (i = 0; i < CAR_BRAIN_QTD_INPUT - 1; i++) {
        ...
        while (1) {
            ...
            if (cenario[(int) X1][(int) Y1] == 0) {
                ...
                double dist = AlgebraLinear::distanciaEntrePontos(carro->X, carro->Y, X1, Y1);
                if (entrada != NULL) {
                    entrada[i] = dist;
                }
...
void DesenharRedeNeural(int X, int Y, int Largura, int Altura, PIG *jogo) {
    ...
    double temp =
            YOrigem - (EscalaAltura * (qtdNeuroEscondidas - 2)) / 2.0 + (EscalaAltura * (qtdNeuroSaida - 1)) / 2.0;

    sprintf(String, "Acelerar");
    jogo->escreverEsquerda(String, X + Largura - 130, temp - 0 * EscalaAltura - 5 - DESLOCAMENTO_NEURONIOS, Fonte);

    sprintf(String, "Re");
    jogo->escreverEsquerda(String, X + Largura - 130, temp - 1 * EscalaAltura - 5 - DESLOCAMENTO_NEURONIOS, Fonte);

    sprintf(String, "Virar Esquerda");
    jogo->escreverEsquerda(String, X + Largura - 130, temp - 2 * EscalaAltura - 5 - DESLOCAMENTO_NEURONIOS, Fonte);

    sprintf(String, "Virar Direita");
    jogo->escreverEsquerda(String, X + Largura - 130, temp - 3 * EscalaAltura - 5 - DESLOCAMENTO_NEURONIOS, Fonte);
    ...
    for (int i = 0; i < qtdEscondidas; i++) {
        ...
        for (int j = 0; j < qtdNeuroEscondidas - 1; j++) {
            ...
            for (int k = 0; k < qtdCamadaAnterior - 1; k++) {
                ...
                if (Peso * Saida > 0) {
                    jogo->desenhaLinhaSimples(XAnterior[k],
                                              YAnterior[k],
                                              NeuroEscondidoX[i][j],
    ...
    for (int i = 0; i < qtdNeuroEntrada - 1; i++) {
        ...
        jogo->desenharSprite(spriteContornoNeuronio,
                             NeuroEntradaX[i],
                             NeuroEntradaY[i],
                             TamanhoNeuronio * 1.1,
                             TamanhoNeuronio * 1.1, 0, 0);

        jogo->desenharSprite(SpriteNeuronAtivado,
                             NeuroEntradaX[i],
                             NeuroEntradaY[i],
                             TamanhoNeuronio,
                             TamanhoNeuronio, 0, 0);
...
void DesenharFrameExplosoes(PIG *jogo) {
    ...
    for (i = 0; i < MAX_EXPLOSOES; i++) {
        if (ListaExplosoes[i].Ativada == 1) {
            ...
            X = ((X + camera->posX) + ((X + camera->posX) - (LARG_TELA / 2.0)) * camera->zoom);
            Y = ((Y + camera->posY) + ((Y + camera->posY) - (ALT_TELA / 2.0)) * camera->zoom);

            jogo->desenharSprite(SpritesExplosao[ListaExplosoes[i].FrameAtual],
                                 X,
                                 Y,
...
void desenhar(PIG *jogo) {
    ...
    jogo->desenharSprite(spriteEstrelasDesligada, LARG_TELA / 2, ALT_TELA / 2, LARG_TELA, ALT_TELA, 0, 0);
    jogo->desenharSprite(spriteEstrelasLigada, LARG_TELA / 2, ALT_TELA / 2, LARG_TELA, ALT_TELA, 0, 0);
    ...
    if (teclado[TECLA_z]) {
        ...
        for (int i = 0; i < QTD_OBSTACULO; i++) {
            ...
            jogo->desenharSprite(spriteEspinho,
                                 X, Y,
        ...
        for (int i = 0; i < QTD_ZONA; i++) {
            ...
            X1 = ((X1 + camera->posX) + ((X1 + camera->posX) - (LARG_TELA / 2.0)) * camera->zoom);
            Y1 = ((Y1 + camera->posY) + ((Y1 + camera->posY) - (ALT_TELA / 2.0)) * camera->zoom);
    ...
    for (int i = 0; i < QTD_CARROS; i++) {
        if (carros[i].Colidiu == 1) {
            ...
            if (carros[i].Queimado == 1) {
                jogo->desenharSprite(spriteCarroQueimado, X, Y, Altura * 2.1, Altura, carros[i].Angulo);
            } else {
                jogo->desenharSprite(carros[i].Sprite, X, Y, Altura * 2.1, Altura, carros[i].Angulo);
            }
        }
    }
    //
    ...
    if (teclado[TECLA_c]) {
        ...
        for (int i = 0; i < CAR_BRAIN_QTD_INPUT - 1; i++) {
            X = carros[indice].X + carros[indice].DistanciaSensores[i] * cos(DEGTORAD * (carros[indice].Angulo - 90.0 +
                                                                                         ((180.0 /
                                                                                           (CAR_BRAIN_QTD_INPUT - 2)) *
                                                                                          i)));
            Y = carros[indice].Y + carros[indice].DistanciaSensores[i] * sin(DEGTORAD * (carros[indice].Angulo - 90.0 +
                                                                                         ((180.0 /
                                                                                           (CAR_BRAIN_QTD_INPUT - 2)) *
                                                                                          i)));
            ...
            XNumero = ((XNumero + camera->posX) + ((XNumero + camera->posX) - (LARG_TELA / 2.0)) * camera->zoom);
            YNumero = ((YNumero + camera->posY) + ((YNumero + camera->posY) - (ALT_TELA / 2.0)) * camera->zoom);
    ...
    if (teclado[TECLA_x]) {
        int MouseX = AlgebraLinear::XVirtualParaReal(event.mouse.posX, event.mouse.posY, camera->cameraRotationZ,
                                                     camera->rotationXY, camera->posX, camera->posY);
        int MouseY = AlgebraLinear::YVirtualParaReal(event.mouse.posX, event.mouse.posY, camera->cameraRotationZ,
            ...
            if (cenario[i][j] == 1) {
                if (distancias[i][j] == valorDesfazerPista) {
                    ...
                    if (3 * (camera->zoom + 1) < 1) {
                        ...
                        jogo->desenhaRetangulo(X, Y, 3 * (camera->zoom + 1), 3 * (camera->zoom + 1), VERDE);
                    }
                }
            }
        }
...
PIG *configuracoesIniciais(const char *arquivoFonte, const char *diretorioImagem, const char *arquivoMatrizDistancias) {
...
//    CriarJanela("Deep Cars", arquivoFonte, 0);
    ...
    Fonte = jogo->criaFonteNormal((char *) arquivoFonte, 15, ESTILO_NORMAL, VERDE, 0, PRETO);
...
int main(int argc, char *args[]) {
    ...
    std::cout << arquivoFonte << " " << diretorioImagem << " " << arquivoMatrizDistancias << std::endl;

    PIG *jogo = configuracoesIniciais(arquivoFonte.c_str(), diretorioImagem.c_str(), arquivoMatrizDistancias.c_str());
