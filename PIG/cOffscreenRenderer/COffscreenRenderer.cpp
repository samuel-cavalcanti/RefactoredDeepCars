...
SDL_Renderer *COffscreenRenderer::GetRenderer() {
    return render;
}

SDL_Surface *COffscreenRenderer::GetSurface() {
    return surface;
}

int COffscreenRenderer::GetAltura() {
    return alt;
}

int COffscreenRenderer::GetLargura() {
    return larg;
}

COffscreenRenderer::COffscreenRenderer(int altura, int largura) {
    alt = altura;
    larg=largura;
...
void COffscreenRenderer::PintarFundo(SDL_Color cor) {
    SDL_SetRenderDrawColor(render,cor.r,cor.g,cor.b,cor.a);
    SDL_RenderClear( render );
}

void COffscreenRenderer::PintarArea(int px, int py, PIG_Cor cor) {
...
void COffscreenRenderer::DesenharLinha(int x1, int y1, int x2, int y2, PIG_Cor cor) {
    SDL_SetRenderDrawColor(render,cor.r,cor.g,cor.b,cor.a);
    SDL_RenderDrawLine(render,x1,alt-y1,x2,alt-y2);
}

void COffscreenRenderer::DesenharRetangulo(int x, int y, int altura, int largura, PIG_Cor cor) {
...
void COffscreenRenderer::DesenharRetanguloVazado(int x, int y, int altura, int largura, PIG_Cor cor) {
    SDL_Rect r={x,alt-y-altura,largura,altura};
...
void COffscreenRenderer::SalvarImagem(char *nomearq) {
