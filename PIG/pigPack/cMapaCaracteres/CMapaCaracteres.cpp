...
CMapaCaracteres::CMapaCaracteres(const char *nomeFonte, SDL_Renderer *renderer, int tamanhoFonte, int estilo,
                                 SDL_Color corFonte, int outline, SDL_Color corOutline) {
    ...
    for (Uint16 j = PRIMEIRO_CAR; j < ULTIMO_CAR; j++) {
        ...
        surfaceTemp[j - PRIMEIRO_CAR] = SDL_CreateRGBSurface(0, larguraLetra[j - PRIMEIRO_CAR],
                                                             alturaLetra[j - PRIMEIRO_CAR], 24, 0, 0, 0, 0);
        ...
        if (CORESIGUAIS(corFonte, BRANCO)) {
            SDL_SetColorKey(surfaceTemp[j - PRIMEIRO_CAR], SDL_TRUE,
                            SDL_MapRGBA(surfaceTemp[j - PRIMEIRO_CAR]->format, 0, 0, 0, 255));//ROXO
        } else
            SDL_SetColorKey(surfaceTemp[j - PRIMEIRO_CAR], SDL_TRUE,
                            SDL_MapRGBA(surfaceTemp[j - PRIMEIRO_CAR]->format, 255, 255, 255, 255));//ROXO
...
CMapaCaracteres::CMapaCaracteres(char *nomeFonte, SDL_Renderer *renderer, int tamanhoFonte, int estilo,
                                 char *fundoFonte, int outline, SDL_Color corOutline) {
    strcpy(nome, nomeFonte);
    tamFonte = tamanhoFonte;
    render = renderer;
...
int CMapaCaracteres::GetLarguraPixelsString(char *str) {
    int resp = 0;
    Uint16 aux;
...
void CMapaCaracteres::EscreveStringCentralizado(char *str, int x, int y) {
    int larguraPixels = GetLarguraPixelsString(str);
    EscreveStringEsquerda(str, x - larguraPixels / 2, y);
}

void CMapaCaracteres::EscreveStringEsquerda(char *str, int x, int y) {
...
void CMapaCaracteres::EscreveStringDireita(char *str, int x, int y) {
    int larguraPixels = GetLarguraPixelsString(str);
